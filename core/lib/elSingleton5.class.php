...
class elSingleton
{
	...
	public static function incLib( $file, $addPath=false )
	{
	  return null != ($p = elSingleton::_incLib(EL_DIR_CORE.$file, $addPath)) ? $p : null;
	}

	public static function _incLib($lib, $addPath)
	{
	...
	public static function createPlugin($name, $pageID, $params)
	{
		$plugin = null;
		$class = 'elPlugin'.$name;
	...
	public static function loadPlugins($pageID)
	{
	...
	public static function loadPlugin($name, $pageID)
	{
		$hash  = md5( $name );
		if (empty($GLOBALS['_elPlugins_'][$hash]))
		{
   ...
   */
	public static function unloadPlugins()
	{
