...
//		$url = $this->_options['URL'].substr($target, strlen($this->_options['root'])+1);	
	...
	function _upload()
	{
		...
		{ 
			if (!empty($_FILES['fm-file']['name'][$i]))
			{
				...
				{
					//$m = elFileInfo::mimetype($_FILES['fm-file']['tmp_name'][$i]);
				...
				else
				{
					...
					if (move_uploaded_file($_FILES['fm-file']['tmp_name'][$i], $target))
					{
						...
						if (elFileInfo::iswebImage($target))
						{
	...
	function _rnd() 
	{
		...
		$this->_te->parse($this->_options['tplName']);
...
//		echo $parts[0];
	...
	function _tmb($img) 
	{
		...
		if (!$this->_imglib) {
			if (strstr(exec('mogrify -version'), 'ImageMagick')) {
				$this->_imglib = 'im';
		...
		if ($this->_imglib == 'gd') 
		{
			if (false == ($info = getimagesize($img['path'])))
			{
				trigger_error('File '.$img['path'].' is not an image', E_USER_WARNING);
				return false;
			}
			$ml = (int)ini_get('memory_limit')*1048576;
			if ( $ml && ($ml/filesize($img['path'])) <= 21 )
			{
				return;
			}
			if (false == ($methods = $this->_gdMethods($img['mimetype'])))
			{
				return;
			}
			...
			if (false === ($image = $methods[0]($img['path'])))
			{
				trigger_error( 'Unable to load image '.$img['path'], E_USER_WARNING);
				return false;
			}
			$width = $height = $this->_options['tmbSize'];
			list($tmbW, $tmbH) = $this->_calcTmbSize($info[0], $info[1], $width, $height, true);

			$thumbnail = imagecreatetruecolor($tmbW, $tmbH);
			if ( !imageCopyResampled($thumbnail, $image, 0,0,0,0, $tmbW, $tmbH, $info[0], $info[1]) )
			{
				trigger_error( 'Unable to resize image '.$img['path'], E_USER_WARNING);
				return false;
			}
			...
			if (1)
			{
				...
				if ($tmbW > $width)
				{
					$x    = intval(($tmbW-$width)/2);
					$tmbW = $width;
				}
				else
				{
					$y = intval(($tmbH-$height)/2);
					$tmbH = $height;
				}
				$canvas = imagecreatetruecolor($width, $height);
				imagecopy($canvas, $thumbnail, 0, 0, $x, $y, $tmbW, $tmbH);
				$result = $methods[1]($canvas, $tmb, $methods[2]);
				imagedestroy($canvas);
			}
			else
			{
				$result = $methods[1]($thumbnail, $tmb, $methods[2]);
			}
			imagedestroy($image);
	...
	function _isAllowed($f, $act) 
	{
		...
		if (isset($this->_options['perms'][$f][$act])) {
			return $this->_options['perms'][$f][$act];
		} elseif (isset($this->_options['defaults'][$act])) {
			return  $this->_options['defaults'][$act];
		}
		return false;
	}
	...
	{
		exit(elJSON::encode(array('error' => vsprintf($tpl, $args))));
	}
	...
	function _jsonMessage($tpl, $args=array())
	{
		...
		$msg  = !$args 
			? $this->_translate($tpl) 
			: $this->_translate( vsprintf( $tpl, $args));
		exit(elJSON::encode(array('message' => $msg)));
	}
	
	function _translate($msg) {
		return $this->_translator ? $this->_translator->translate($msg) : $msg;
	}
	...
	function _gdMethods($mime)
	{
		switch ($mime)
		{
			case 'image/jpeg': return imagetypes() & IMG_JPG ? array('imagecreatefromjpeg', 'imagejpeg', 98)   : false;
	 ...
	 * @author dio
	 **/
	function _calcTmbSize($imgW, $imgH, $tmbW, $tmbH, $max=false)
	{
		if ( $imgW >= $imgH )
		{
			if (!$max)
