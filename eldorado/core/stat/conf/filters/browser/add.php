 ...
 ****************************************************************/
 ...
 function local_check() {
	 ...
	 if (!eregi("^[А-Яа-яA-Za-z0-9() -<=>]+$", !empty($_POST['text']))) {
		 $error.="Введите название.<br>\n";
	 }

	 if (!eregi("^[А-Яа-яA-Za-z0-9() -<=>]+$", !empty($_POST['name']))) {
		 $error.="Введите значение.<br>\n";
	 }

	 if (!eregi("^[А-Яа-яA-Za-z0-9() -<=>]+$", !empty($_POST['val']))) {
 ...
 if (isset($_POST) && $action == "add") {
	 ...
	 if ($error == "") {
		 ...
		 $core->exec("INSERT INTO wc_browser (name, value, text) VALUES ('$name', '$val', '$text');");
 ...
 } elseif (!empty($_POST) && $action == "edit") {
	 ...
	 if ($error == "") {
		 ...
		 $core->exec("UPDATE wc_browser SET name='$name', value='$val', text='$text' WHERE id=$id LIMIT 1;");
	 ...
	 $res=$core->exec("SELECT * FROM wc_browser WHERE id=$id LIMIT 1;");
