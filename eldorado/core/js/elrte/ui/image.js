...
elRTE.prototype.ui.prototype.buttons.image = function(rte, name) {
		...
		this.src = {
			main : {
				...
				align  : $('<select />').css('width', '100%')
							.append($('<option />').text(this.rte.i18n('Not set', 'dialogs')))
	...
	this.command = function() {
		...
		if (n.nodeName == 'IMG') {
			this.img     = $(n);
			this.prevImg = this.img.clone();
			this.prevImg.attr('src', this.rte.utils.absoluteURL(this.prevImg.attr('src')));
			if (n.parentNode.nodeName == 'A' && n.parentNode.href) {
				this.link = $(n.parentNode);
			}
		} else {
		...
		this.prevImg && this.preview.prepend(this.prevImg) && this.updateValues();
	...
	this.set = function() {
		if (!this.prevImg || !this.prevImg.attr('width')) {
			...
			if (!this.img.parents().length) {
				...
				this.rte.doc.execCommand('insertimage', false, fakeURL);
				this.img = $('img[src="'+fakeURL+'"]', this.rte.doc);
			}
