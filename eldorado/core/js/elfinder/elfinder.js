(function($) {
	...
	$.fn.elfinder = function(o) {
		...
		var viewVariants = ['list', 'ismall', 'ibig'];
		...
		return this.each(function() {
			...
			var id       = $self.attr('id');
			/* Куки для хранения выбраного вида деректории */
			this.vCookie = 'el-fm-view-'+id;
			var view     = $.cookie(this.vCookie);
			if (view && $.inArray(view, viewVariants)) {
				this.view = view;
			} else {
				this.view = 'list';
				$.cookie(this.vCookie, this.view, {expires: 1});
			}
			/* Куки, хранят инф - отображать или нет отчеты о выполненных командах */
			this.rCookie     = 'el-fm-reports3-'+id;
			/* флаг - отображать или нет отчеты о выполненных командах */
			this.showReports = parseInt($.cookie(this.rCookie));
			if (isNaN(this.showReports)) {
				this.showReports = 1;
				$.cookie(this.rCookie, this.showReports, {expires: 1});
			}
			...
			this.init = function(html) {
				...
				this.cwd = $self.find('.el-finder-cwd')
					...
					.bind('update', function() {
						...
						$.elcontextmenu(selectors, self.cwd, function(e) { 
				...
				this.nav = $self.find('.el-finder-nav')
					.bind('update', function() {
						...
						self.tree.find('[key="'+self.info.key+'"]').trigger('expand');
						self._unlock();
					});
			...
			**/
			this.updateToolbar = function() {
				...
				for (var i=0; i < viewVariants.length; i++) {
					var view = viewVariants[i];
					if (view == self.view) {
			...
			**/
			this.swithchView = function(v) {
				self.view = v;
				$.cookie(self.vCookie, v, {expires: 1});
				self.reload();
			}
			...
			**/
			this.switchReports = function() {
				self.showReports = self.showReports == 0 ? 1 : 0;
				$.cookie(self.rCookie, self.showReports, {expires: 1});
			...
			this.help = function() {
				...
				var h = '<strong>'+self.i18n('elFinder: Web file manager')+' '+self.i18n('Version')+': '+version+'</strong><br />';
				var a = self.i18n('Copyright: Studio 42 LTD')+'<br />'
						+self.i18n('Programming: Dmitry (dio) Levashov, dio@std42.ru')+' 2009<br />'
						+self.i18n('Design: Valentin Razumnih')+'<br />'
						+self.i18n('License: BSD License')+'<br /><a href="http://www.elrte.ru">www.elrte.ru</a>'
			...
			this._selected = function(first) {
				...
				return null;
				return first ? self.cwd.find('.selected').eq(0) : self.cwd.find('.selected');
			}
